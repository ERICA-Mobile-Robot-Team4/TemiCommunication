...
public class ResultActivity extends AppCompatActivity implements OnRobotReadyListener {

    private static final String CULPRIT = "강병철";

    private static final String TTS_SUCCESS =
        "수사 결과가 확인되었습니다. " +
        "강병철, 당신을 블랙우드 회장 살인 혐의로 검거합니다. " +
        "수고하셨습니다. 블랙우드 저택의 밤이 밝았습니다.";

    private static final String TTS_FAILURE =
        "검거 결과, 무고한 시민으로 판명되었습니다. " +
        "진범은 아직 저택 어딘가에 있습니다. " +
        "단서를 다시 살펴보고, 수사를 재개하십시오.";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isCorrect) {
            ...
            pendingSpeech = TTS_FAILURE;
        }

        // 1초 딜레이 후 TTS (화면이 먼저 보이도록)
        new Handler().postDelayed(() -> speakResult(), 1000);
    ...
    public void onRobotReady(boolean isReady) {
        if (isReady && pendingSpeech != null) {
            speakResult();
        }
    }

    private void speakResult() {
        if (robot != null && pendingSpeech != null) {
            robot.speak(com.robotemi.sdk.TtsRequest.create(pendingSpeech, false));
        }
    }
