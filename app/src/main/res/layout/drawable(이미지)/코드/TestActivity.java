...
public class TestActivity extends AppCompatActivity implements OnRobotReadyListener {
    ...
    private LinearLayout layoutClues;
    ...
    private void bindViews() {
        ...
        layoutClues = findViewById(R.id.layoutClues);
    ...
    private void setupClueCards() {
        layoutClues.removeAllViews();

        addClueCard(
                "Evidence #01",
                "촛대 손잡이의 섬유",
                "흉기로 추정되는 촛대 손잡이에서 미세한 면 섬유가 발견됨."
        );

        addClueCard(
                "Evidence #02",
                "조작된 창문 발자국",
                "실제 침입보다 신발을 손으로 눌러 찍은 흔적에 가까움."
        );

        addClueCard(
                "Evidence #06",
                "비어 있는 설거지 기록",
                "21시 50분부터 23시 10분까지 주방 작업 기록이 비어 있음."
        );

        addClueCard(
                "Evidence #08",
                "주방 뒷문 개폐 기록",
                "22시 28분에 열렸다가 22시 34분에 다시 잠김."
        );

        addClueCard(
                "Evidence #12",
                "회중시계 보증서와 S-2 열쇠",
                "수령인 첫 글자가 희미하게 강으로 보이고, 서재 옆 보조문 열쇠가 발견됨."
        );

        tvEvidenceCount.setText(acquiredClues.size() + "/12 FOUND");
    }

    private void addClueCard(String label, String title, String desc) {
        LinearLayout card = new LinearLayout(this);
        card.setOrientation(LinearLayout.VERTICAL);
        card.setPadding(10, 8, 10, 8);
        setRoundBg(card, Color.parseColor("#20242E"), Color.parseColor("#3A3030"), 2);
        ...
        TextView tvLabel = new TextView(this);
        tvLabel.setText(label);
        tvLabel.setTextColor(Color.parseColor("#8A4E4E"));
        tvLabel.setTextSize(8);
        tvLabel.setGravity(Gravity.END);
        tvLabel.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        tvDesc.setText(desc);
        tvDesc.setTextColor(Color.parseColor("#9EA3AD"));
        tvDesc.setTextSize(8);
        tvDesc.setLineSpacing(2, 1.0f);

        card.addView(tvLabel);
        card.addView(tvTitle);
        card.addView(tvDesc);

        layoutClues.addView(card);
    }
